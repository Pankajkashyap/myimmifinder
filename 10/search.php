  ...
  <div style="display:none" id="loader"><img src="img/spinner.gif"></div>
	<div class="container">
		...
		$(function(){
				...
				$.ajax({
					...
					success: function(res){
						if(res.status  == true){
							...
							$( '#ajaxify' ).html( res.table );

							var tbody = $( '#mytable tbody' ), props = res.arraykey;

							$.each( mydata, function(i, value){
								var tr = $('<tr>');
								$.each(props, function(i, prop){
									if(i==0)
									$('<td class="'+arr[i]+' statehead">').html(value[prop]).appendTo(tr);
									else
									$('<td class="'+arr[i]+'">').html(value[prop]).appendTo(tr);
								});
				...
				$.ajax({
					...
					success: function(res){
						if(res.status  == true){
							...
							var tbody = $( '#mytable1 tbody' ), props = res.arraykey;
							...
							$('#mytable1').table().data( "table" ).refresh();
						}
					}
				});
			});
			...
			<table class="tablesaw table table-hover sss" data-tablesaw-mode="swipe" data-tablesaw-sortable data-tablesaw-minimap id="mytable">
				<thead>
					<tr>
						<th scope="col" class="tablesaw-cell-persist" data-tablesaw-priority="persist">State</th>
				...
				<tbody>
						...
						<?php
							$getHospitals = $conn->getHospital($_GET['destination']);
							$getHospitals = array_slice($getHospitals, 0, 1);
							foreach($getHospitals as $getHospital){
						?>
						<tr class="ddd">
